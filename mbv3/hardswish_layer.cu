...
__global__ void HardSwishForward(const int n, const Dtype* in, Dtype* out) {
  CUDA_KERNEL_LOOP(index, n) {
    ...
    out[index] = in[index] <-3.0 ? 0 :(in[index] >3.0 ? in[index] : in[index]*(in[index]/6.0 + 0.5));
  }
}
...
__global__ void HardSwishBackward(const int n, const Dtype* in_diff,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    out_diff[index] = in_diff[index] * (in_data[index]<-3.0 ? 0:(in_data[index]>3.0 ? 1.0:in_data[index]/3.0+0.5));
  }
}
