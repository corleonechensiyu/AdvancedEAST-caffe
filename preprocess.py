...
def resize_image(im, max_img_size=cfg.max_train_img_size):
    im_width = np.minimum(im.width, max_img_size)#####min(1280,640)=640
    if im_width == max_img_size < im.width:
        im_height = int((im_width / im.width) * im.height)####640/1280 * 720==360
    else:
        im_height = im.height#### 300
    o_height = np.minimum(im_height, max_img_size) ####min(360,640)===360
    if o_height == max_img_size < im_height:
        o_width = int((o_height / im_height) * im_width)
    else:
        o_width = im_width###360
    d_wight = o_width - (o_width % 32)####640-0=640
    d_height = o_height - (o_height % 32)#####360-8=352
