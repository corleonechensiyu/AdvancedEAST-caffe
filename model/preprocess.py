...
import cv2
import cfg
from model.label import shrink
...
def resize_image(im, max_img_size=cfg.max_train_img_size):
    im_width = np.minimum(im.shape[1], max_img_size)#####min(475,256)=256
    if im_width == max_img_size < im.shape[1]:
        im_height = int((im_width / im.shape[1]) * im.shape[0])####256/475 * 300==161
    else:
        im_height = im.shape[0]#### 300
...
def preprocess():
    ...
    for o_img_fname, _ in zip(o_img_list, tqdm(range(len(o_img_list)))):
        ...
        scale_ratio_w = d_wight / im.shape[1]
        scale_ratio_h = d_height / im.shape[0]
        im = cv2.resize(im, (d_wight, d_height))
        show_gt_im = im.copy()
        # draw on the img
        ...
        for anno, i in zip(anno_list, range(len(anno_list))):
            ...
            if draw_gt_quad:
                for i in range(1, len(xy_list)):
                    # green
                    cv2.line(show_gt_im, (int(round(xy_list[i - 1][0])), int(round(xy_list[i - 1][1]))),
                             (int(round(xy_list[i][0])), int(round(xy_list[i][1]))), (0, 255, 0), 2,
                             lineType=cv2.LINE_AA)

                    # blue
                    cv2.line(show_gt_im,
                             (int(round(shrink_xy_list[i - 1][0])), int(round(shrink_xy_list[i - 1][1]))),
                             (int(round(shrink_xy_list[i][0])), int(round(shrink_xy_list[i][1]))), (255, 0, 0), 2,
                             lineType=cv2.LINE_AA)

                cv2.line(show_gt_im, (int(round(xy_list[3][0])), int(round(xy_list[3][1]))),
                         (int(round(xy_list[0][0])), int(round(xy_list[0][1]))), (0, 255, 0), 2,
                         lineType=cv2.LINE_AA)
                cv2.line(show_gt_im, (int(round(shrink_xy_list[3][0])), int(round(shrink_xy_list[3][1]))),
                         (int(round(shrink_xy_list[0][0])), int(round(shrink_xy_list[0][1]))), (255, 0, 0), 2,
                         lineType=cv2.LINE_AA)
                ...
                for q_th in range(2):
                    cv2.line(show_gt_im, (
                        int(round(xy_list[vs[long_edge][q_th][0]][0])), int(round(xy_list[vs[long_edge][q_th][0]][1]))),
                             (int(round(shrink_1[vs[long_edge][q_th][1]][0])),
                              int(round(shrink_1[vs[long_edge][q_th][1]][1]))),
                             (0, 255, 255), 2, lineType=cv2.LINE_AA)
                    cv2.line(show_gt_im, (
                        int(round(shrink_1[vs[long_edge][q_th][1]][0])),
                        int(round(shrink_1[vs[long_edge][q_th][1]][1]))),
                             (int(round(shrink_1[vs[long_edge][q_th][2]][0])),
                              int(round(shrink_1[vs[long_edge][q_th][2]][1]))),
                             (0, 255, 255), 2, lineType=cv2.LINE_AA)
                    cv2.line(show_gt_im, (
                        int(round(shrink_1[vs[long_edge][q_th][2]][0])),
                        int(round(shrink_1[vs[long_edge][q_th][2]][1]))),
                             (int(round(xy_list[vs[long_edge][q_th][3]][0])),
                              int(round(xy_list[vs[long_edge][q_th][3]][1]))),
                             (0, 255, 255), 2, lineType=cv2.LINE_AA)
                    cv2.line(show_gt_im, (
                        int(round(xy_list[vs[long_edge][q_th][3]][0])),
                        int(round(xy_list[vs[long_edge][q_th][3]][1]))),
                             (int(round(xy_list[vs[long_edge][q_th][4]][0])),
                              int(round(xy_list[vs[long_edge][q_th][4]][1]))),
                             (0, 255, 255), 2, lineType=cv2.LINE_AA)
                    cv2.line(show_gt_im, (
                        int(round(xy_list[vs[long_edge][q_th][4]][0])),
                        int(round(xy_list[vs[long_edge][q_th][4]][1]))),
                             (int(round(xy_list[vs[long_edge][q_th][0]][0])),
                              int(round(xy_list[vs[long_edge][q_th][0]][1]))),
                             (0, 255, 255), 2, lineType=cv2.LINE_AA)
        if cfg.gen_origin_img:
            cv2.imwrite(os.path.join(train_image_dir, o_img_fname),im)
        ...
        if draw_gt_quad:
            ...
            cv2.imwrite(os.path.join(show_gt_image_dir, o_img_fname),show_gt_im)
